			...
			<div className="gyp__audio-player-3d max-w-full bg-white border-black border-2 shadow-lg flex flex-col justify-center items-center p-4">
				...
				<div className="flex flex-row">
					...
					{state.isPlaying === true &&
						<svg onClick={() => { dispatch({ type: ACTIONS.pause, payload: { audio: audio, image: image }}) }} className="w-12 h-12 fill-none hover:fill-slate-300" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
							<path strokeLinecap="round" strokeLinejoin="round" d="M14.25 9v6m-4.5 0V9M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
						</svg>
					}
					{state.isPlaying === false &&
						<svg onClick={() => { dispatch({ type: ACTIONS.play, payload: { audio: audio, image: image }}) }} className="w-12 h-12 fill-none hover:fill-slate-300" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
							<path strokeLinecap="round" strokeLinejoin="round" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
							<path strokeLinecap="round" strokeLinejoin="round" d="M15.91 11.672a.375.375 0 010 .656l-5.603 3.113a.375.375 0 01-.557-.328V8.887c0-.286.307-.466.557-.327l5.603 3.112z" />
						</svg>
					}
					<svg onClick={() => { dispatch({ type: ACTIONS.playNext, payload: audio, audioObjects: AUDIO_OBJECTS }) }} className={`w-12 h-12 fill-none hover:fill-slate-300 ${state.disabledNext ? "text-gray-600" : "text-black"}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
